...
Fitting a model with the transmission type, weight, horsepower and number of cylinders as predictors, we could not find a statistically significant relationship between transmission type and fuel efficiency as measured in miles per gallon.
Then again, the study can be repeated with a better database that includes more vehicles.
...
```
While there seems to be a small increase in fuel efficiency (1.8 miles per gallon) for manual transmission cars, the confidence interval includes 0. The p-value is 0.21, meaning there is a 21% chance of getting this result if we consider that there is no influence of the predictor on the outcome.
...
summary(mdl)$coef
```
Now we see that for a fixed number of cylinders, horsepower and weight, the change from automatic to manual transmission might increase the fuel efficiency by 1.8 miles per gallon. We can create several models and test the additional information each predictor adds to the model.
...
We see that there are some models influencing the model, such as *Toyota Corolla*, *Toyota Corona*, *Chrysler Imperial* and *Datsuo 710*. We can try to remove those points and evaluate the effect over the model.

```{r}
 data$car <- rownames(data)
data1 <- data[!(data$car %in% c('Toyota Corolla','Toyota Corona','Chrysler Imperial','Datsuo 710')),]
